...
We did the hunt with **Censys** and **Shodan** (free versions). Think Google, but instead of pages, they return metadata that gives away connected services and devices - just what we need to profile goals.
...
* Make the browser point to the ZAP/Burp **Local ProxyHTTP** (e.g., `127.0.0.1:8080`).
...
> This chapter is about **applied methodology**: how we take what we find “in the hunt” and lower it to Colander.
...
The Graph functionality in Colander gives us a better idea of what the relationships look like, the following blunder corresponds to a __sub-graph__ of this specific threat:
...
Colander has a key piece to “get out” what we found and **use it**: the **feeds**. Here we are going to use **feeds of entities** (not rules, that's for another day). By “entities” we mean what Colander models in the UI: **Actors, Artifacts, Devices, Observables, Threats**. The idea: export **Observables** and **Threats** from this case, download them in **STIX2** and point **MVT** to that file as a source of IOCs.

> It is not a Colander course: here we go to the minimum operational. Whoever wants to learn well, to the PTS docs. This chapter is “how we do it”.
...
4. **Secret:** is the **key** of the feed. It is **mandatory** to access.

   * You can have **several feeds** with different content and different **secrets** (URL + secret shares depending on who you are with).
   * You can **rotate** the secret whenever you want (invalidate the previous one).

5. **TLP / PAP** (the two most confusing fields at the beginning):
...
> Be careful with the **URL**: Colander usually includes parameters with special characters; **put the URL in quotes** or your shell will trip.
...
Expected output : MVT detects the **domain** we documented in the case (e.g., the Spanish domain), and —if your feed includes it in STIX2 with relationships— you will see the flag labeled with the **name/label** of the associated **Threat**.
